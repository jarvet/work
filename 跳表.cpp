...
struct node* Delete(struct node *top, int data)
{
    if(top->next->value == data && top->next->next->next == NULL)
    {
    ...
    {
        struct node *p_next = p_top;  //依次横向下一个
        while(data != p_next->next->value && p_next->next->value != 1)
        {
            p_next = p_next->next;
        }
        if(data == p_next->next->value)
        {
            struct node* dele = p_next->next;
            p_next->next = dele->next;
            free(dele);
        }
        p_top = p_top->down;
